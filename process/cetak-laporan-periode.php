...
// Ambil data pengaduan berdasarkan periode
...
$pdf->SetFont('Arial','',9);
$pdf->Cell(0,5,'Periode: '.date('d-m-Y',strtotime($tanggal_awal)).' s/d '.date('d-m-Y',strtotime($tanggal_akhir)),0,1,'L');
...
if(mysqli_num_rows($result) > 0) {
  while($data = mysqli_fetch_assoc($result)) {
    ...
    $pdf->Cell(30,6,date('d-m-Y',strtotime($data['tanggal'])),1,0,'C',$fill);
  ...
  $pdf->Cell(180,6,'Tidak ada data pengaduan pada periode tersebut',1,1,'C');
}
...
$menunggu = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM input_aspirasi ia LEFT JOIN aspirasi a ON ia.id_pelaporan = a.id_pelaporan WHERE ia.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND a.status='Menunggu'"));
$proses = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM input_aspirasi ia LEFT JOIN aspirasi a ON ia.id_pelaporan = a.id_pelaporan WHERE ia.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND a.status='Proses'"));
$selesai = mysqli_num_rows(mysqli_query($conn, "SELECT * FROM input_aspirasi ia LEFT JOIN aspirasi a ON ia.id_pelaporan = a.id_pelaporan WHERE ia.tanggal BETWEEN '$tanggal_awal' AND '$tanggal_akhir' AND a.status='Selesai'"));
...
// Output PDF
$pdf->Output('I', 'Laporan_Periode_'.date('Ymd',strtotime($tanggal_awal)).'_'.date('Ymd',strtotime($tanggal_akhir)).'.pdf');
?>
